...
-- and find their percentage from total customers
SELECT 
	Gender, 
...
SELECT 
	...
	ROUND (COUNT(Contract) * 100.0 / (SELECT COUNT(*) FROM customer_churn), 2)  AS Percentage
from customer_churn
Group by Contract;

-- Change Data Type.
ALTER TABLE customer_churn
ALTER COLUMN total_revenue TYPE float
USING total_revenue::float;


-- Calculate total customers and total revenue
-- for each customer status category
SELECT 
    Customer_Status,
    ...
    (SELECT SUM(Total_Revenue) FROM customer_churn)
    AS RevPercentage
...
-- This query analyzes customer distribution by state
SELECT 
    State, 
...
-- I created SQL views to provide filtered datasets for Power BI reporting. 
-- Separate views were built for churn analysis and new joiner analysis to 
-- simplify dashboard development and improve query reusability.
